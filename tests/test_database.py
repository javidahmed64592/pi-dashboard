...
from pi_dashboard.models import NoteAction, NoteEntry
...
class TestDatabaseManager:
    ...
    def test_perform_note_action_create(
        ...
        note_id = mock_database_manager.perform_note_action(mock_note_entry_2, NoteAction.CREATE)
    ...
    def test_perform_note_action_update(
        ...
        note_id = mock_database_manager.perform_note_action(mock_note_entry_2, NoteAction.UPDATE)
        assert isinstance(note_id, int)
    ...
    def test_perform_note_action_update_not_found(
        ...
        with pytest.raises(ValueError, match=f"Note entry with ID {mock_note_entry_2.id} not found for update."):
            mock_database_manager.perform_note_action(mock_note_entry_2, NoteAction.UPDATE)
    ...
    def test_perform_note_action_delete(self, mock_database_manager: DatabaseManager) -> None:
        ...
        note_id = mock_database_manager.perform_note_action(note_to_delete, NoteAction.DELETE)
        assert isinstance(note_id, int)
    ...
    def test_perform_note_action_delete_not_found(
        ...
        with pytest.raises(ValueError, match=f"Note entry with ID {mock_note_entry_2.id} not found for deletion."):
            mock_database_manager.perform_note_action(mock_note_entry_2, NoteAction.DELETE)
