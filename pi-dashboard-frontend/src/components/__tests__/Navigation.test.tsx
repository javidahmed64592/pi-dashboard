...
describe("Navigation", () => {
  ...
  it("renders navigation links with correct hrefs", () => {
    ...
    const systemLinks = screen.getAllByRole("link", {
      name: "System",
    });
    expect(systemLinks[0]).toHaveAttribute("href", "/system");
  });
  ...
  it("applies active styling to current page", () => {
    ...
    expect(dashboardLinks[0]).toHaveClass("font-bold", "text-border-accent");
    expect(dashboardLinks[1]).toHaveClass("font-bold", "text-border-accent");
  });
  ...
  describe("Mobile Navigation", () => {
    ...
    it("closes mobile menu when navigation link is clicked", () => {
      ...
      const dashboardLink = dashboardLinks[1]; // Second one is in mobile menu
      if (dashboardLink) {
    ...
    it("applies active styling to current page in mobile menu", () => {
      ...
      const mobileDashboardLink = dashboardLinks[1]; // Second one is in mobile menu
      expect(mobileDashboardLink).toHaveClass(
        "font-bold",
        "text-border-accent"
      );
    });
  ...
  describe("HealthIndicator integration", () => {
    ...
    it("positions HealthIndicator next to the logo", () => {
      ...
      expect(nav).toContainElement(screen.getByText("Pi Dashboard"));
